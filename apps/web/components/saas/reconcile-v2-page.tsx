...
export function ReconcileDashboardPage() {
  ...
  return <AppShell title="2A/2B Reconcile" subtitle="Professional ITC reconciliation across GST portal 2A/2B and purchase books." profile={workspace.profile} profiles={workspace.profiles} token={workspace.token} user={workspace.user} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }} actions={<Link href="/modules/reconcile/upload" className="inline-flex items-center gap-2 rounded-2xl bg-[#10244d] px-5 py-3 text-sm font-bold text-white"><UploadCloud className="size-4" /> New reconcile</Link>}>
...
export function ReconcileUploadPage() {
  ...
  return <AppShell title="Upload & Reconcile" subtitle="Upload GST portal 2A/2B and purchase register files, tune tolerances, then generate match results." profile={workspace.profile} profiles={workspace.profiles} token={workspace.token} user={workspace.user} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }} actions={<a href="data:text/csv;charset=utf-8,Supplier GSTIN,Invoice Number,Invoice Date,Taxable Value,IGST,CGST,SGST%0A" download="gst-bharat-2a-2b-sample.csv" className="inline-flex items-center gap-2 rounded-2xl bg-[#10244d] px-5 py-3 text-sm font-bold text-white"><FileSpreadsheet className="size-4" /> Sample CSV</a>}>
    <div className="space-y-6">
...
export function ReconcileHistoryPage() {
  ...
  return <AppShell title="Reconciliation History" subtitle="Download center for previous 2A/2B matching runs." profile={workspace.profile} profiles={workspace.profiles} token={workspace.token} user={workspace.user} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }}>
