...
import { deleteTransaction, getProfileTransactions, updateTransaction, Transaction } from "@/lib/api";
...
export function TransactionsPage() {
  ...
  useEffect(() => {
    ...
    getProfileTransactions(workspace.token, workspace.profile.id)
      ...
      .catch((exc) => {
        if (!cancelled) {
          setAllRows([]);
          setActionError(exc instanceof Error ? exc.message : "Could not load transactions.");
        }
      })
      .finally(() => {
  ...
  const sourceRows = allRows.length ? allRows : workspace.transactions;
  const periods = Array.from(new Set(sourceRows.map((row) => row.filing_period).filter(Boolean))).sort().reverse();
  const platforms = Array.from(new Set(sourceRows.map((row) => row.platform))).sort();
  const states = Array.from(new Set(sourceRows.map((row) => row.buyer_state_code).filter(Boolean))).sort();
  const rates = Array.from(new Set(sourceRows.map((row) => String(row.gst_rate)))).sort();
  const activePeriodRows = sourceRows.filter((row) => row.filing_period === workspace.profile?.return_period);
  const rows = useMemo(() => sourceRows.filter((row) =>
    JSON.stringify(row).toLowerCase().includes(query.toLowerCase()) &&
    (period === "all" || row.filing_period === period) &&
  ...
  ), [sourceRows, query, period, platform, docType, rate, state, errorOnly]);
  ...
  async function remove(row: Transaction) {
    ...
    try {
      ...
      if (workspace.profile) setAllRows(await getProfileTransactions(workspace.token, workspace.profile.id));
    } catch (exc) {
  ...
  return (
    <AppShell requiresSubscription requiredPlan="online_seller" token={workspace.token} user={workspace.user} productName="GST Online Seller" title="Transactions" subtitle="Normalized transaction database with professional filters, inline correction and detail drawer." profile={workspace.profile} profiles={workspace.profiles} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }} actions={<button onClick={exportCsv} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#10244d] px-5 py-3 text-sm font-bold text-white sm:w-auto"><Download className="size-4" /> Export CSV</button>}>
      ...
      <Panel title="Merged transaction table" subtitle={loadingRows ? "Loading all profile periods..." : `${rows.length} of ${sourceRows.length} rows visible`}>
        {workspace.profile && !workspace.transactions.length && activePeriodRows.length === 0 && sourceRows.length > 0 ? <div className="mb-4 rounded-2xl bg-amber-50 p-4 text-sm font-bold text-amber-800">No rows found for active return period {periodLabel(workspace.profile.return_period)}. Showing rows from other uploaded periods below.</div> : null}
        <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
          <input value={query} onChange={(event) => setQuery(event.target.value)} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm sm:col-span-2 dark:border-white/10 dark:bg-slate-900" placeholder="Search invoices, orders, SKU" />
          <select value={period} onChange={(event) => setPeriod(event.target.value)} className="rounded-2xl border border-slate-200 px-3 py-3 text-sm dark:border-white/10 dark:bg-slate-900"><option value="all">All periods</option>{periods.map((item) => <option key={item} value={item}>{periodLabel(item)}</option>)}</select>
        ...
        {rows.length ? <div className="table-scroll max-h-[620px] overflow-auto rounded-2xl border border-slate-200 dark:border-white/10 sm:rounded-3xl"><table className="min-w-[1360px] text-left text-sm"><thead className="sticky top-0 bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900"><tr>{["Period", "Platform", "Invoice", "Order", "Date", "POS", "HSN", "Taxable", "Rate", "IGST", "CGST", "SGST", "Doc", "Status", ""].map((head) => <th key={head} className="px-4 py-3">{head}</th>)}</tr></thead><tbody className="divide-y divide-slate-100 dark:divide-white/10">{rows.map((row) => <tr key={row.id} className="bg-white hover:bg-slate-50 dark:bg-slate-950 dark:hover:bg-white/5"><td className="px-4 py-3 font-bold">{periodLabel(row.filing_period)}</td><td className="font-bold capitalize">{row.platform}</td><td>{row.invoice_no}</td><td>{row.order_id}</td><td>{row.invoice_date}</td><td><input defaultValue={row.buyer_state_code || ""} onBlur={(event) => inlineUpdate(row, "buyer_state_code", event.target.value)} className="w-14 rounded-lg border px-2 py-1 dark:border-white/10 dark:bg-slate-900" /></td><td>{row.hsn}</td><td>{formatCurrency(row.taxable_value)}</td><td>{row.gst_rate}%</td><td>{formatCurrency(row.igst)}</td><td>{formatCurrency(row.cgst)}</td><td>{formatCurrency(row.sgst)}</td><td>{row.doc_type}</td><td><StatusPill status={row.validation_status} /></td><td className="flex gap-2 px-4 py-3"><button onClick={() => setDetail(row)} className="rounded-xl bg-slate-100 p-2 dark:bg-white/10"><Eye className="size-4" /></button><button onClick={() => remove(row)} className="rounded-xl bg-rose-50 p-2 text-rose-600"><Trash2 className="size-4" /></button></td></tr>)}</tbody></table></div> : <EmptyState title="No transactions found" body="Upload marketplace files or clear filters to see normalized rows." />}
      </Panel>
      {detail && <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40" onClick={() => setDetail(null)}><aside onClick={(event) => event.stopPropagation()} className="h-full w-full max-w-xl overflow-auto bg-white p-4 shadow-2xl dark:bg-slate-950 sm:p-6"><h2 className="text-2xl font-black">Row details</h2><p className="mt-1 break-words text-sm text-slate-500">{detail.invoice_no || detail.order_id}</p><pre className="mt-6 overflow-auto whitespace-pre-wrap rounded-2xl bg-slate-950 p-4 text-xs text-slate-100 sm:rounded-3xl sm:p-5">{JSON.stringify(detail, null, 2)}</pre></aside></div>}
    </AppShell>
  );
}
