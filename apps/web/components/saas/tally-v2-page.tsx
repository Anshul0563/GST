...
export function TallyDashboardPage() {
  ...
  return <AppShell requiresSubscription requiredPlan="ecom_tally" token={workspace.token} user={workspace.user} productName="eCom to Tally" title="eCom to Tally" subtitle="Convert normalized marketplace transactions into Tally-compatible XML vouchers." profile={workspace.profile} profiles={workspace.profiles} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }} actions={<Link href="/modules/tally/export" className="inline-flex items-center gap-2 rounded-2xl bg-[#10244d] px-5 py-3 text-sm font-bold text-white"><FileArchive className="size-4" /> Generate XML</Link>}>
...
export function TallyCompanyPage() {
  ...
  return <AppShell requiresSubscription requiredPlan="ecom_tally" token={workspace.token} user={workspace.user} productName="eCom to Tally" title="Tally Company" subtitle="Create or select Tally company details used during XML generation." profile={workspace.profile} profiles={workspace.profiles} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }}>
...
export function TallyImportPage() {
  ...
  return <AppShell requiresSubscription requiredPlan="ecom_tally" token={workspace.token} user={workspace.user} productName="eCom to Tally" title="Tally Import" subtitle="Upload marketplace files directly into the shared normalized transaction engine." profile={workspace.profile} profiles={workspace.profiles} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }}>
...
export function TallyExportPage() {
  ...
  return <AppShell requiresSubscription requiredPlan="ecom_tally" token={workspace.token} user={workspace.user} productName="eCom to Tally" title="Generate Tally XML" subtitle="Preview mapping, generate XML and download voucher Excel." profile={workspace.profile} profiles={workspace.profiles} loading={workspace.loading} error={workspace.error} onRetry={() => workspace.refresh()} onProfileChange={(profile) => { workspace.setProfile(profile); workspace.refresh(profile); }}>
