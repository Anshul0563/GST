...
import { b2cs } from "@/lib/mock-data";
  ...
  return (
    <Card id="gstr-1-json">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button><Download className="size-4" />Download JSON</Button>
      </CardHeader>
      <CardContent className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        ...
        <div className="space-y-3">
          {["SUPECO summary: 3 operator groups", "Document issue: invoice and credit note ranges ready", "Platform contribution: Meesho 18%, Amazon 33%, Flipkart 49%", "State-wise GST summary ready"].map((item) => <div key={item} className="rounded-lg bg-slate-50 p-3 text-sm text-slate-700">{item}</div>)}
