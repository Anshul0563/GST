...
def money(value: object) -> Decimal:
    ...
    if isinstance(value, Decimal):
        ...
        is_cr_dr_negative = upper.endswith("CR") or upper.endswith("DR")
        multiplier = (
            Decimal("-1")
            if is_parenthesized_negative or is_cr_dr_negative
            else Decimal("1")
        )

        cleaned = upper.replace("CR", "").replace("DR", "").strip()
...
def validate_transaction(txn: dict) -> list[str]:
    ...
    if (
        doc_type in {"invoice", "credit_note", "debit_note"}
        and not txn.get("invoice_no")
    ):
    ...
    igst = money(txn.get("igst"))
    cgst = money(txn.get("cgst"))
    sgst = money(txn.get("sgst"))
    cess = money(txn.get("cess"))

    if (
        taxable == Decimal("0.00")
        and igst == Decimal("0.00")
        and cgst == Decimal("0.00")
        and sgst == Decimal("0.00")
        and cess == Decimal("0.00")
    ):
