...
MARKETPLACE_CATALOG = {
    ...
    "ajio": {
        ...
        "category": "Fashion",
        "required_files": ["Sales report"],
        "guide": "Upload AJIO seller tax report in the common GST Bharat schema",
    },
    "tatacliq": {
        "name": "Tata CLiQ",
        "category": "Ecommerce",
        "required_files": ["Sales report"],
        "guide": "Upload Tata CLiQ seller tax report in the common GST Bharat schema",
    },
    "nykaa": {
        "name": "Nykaa",
        "category": "Beauty",
        "required_files": ["Sales report"],
        "guide": "Upload Nykaa seller tax report in the common GST Bharat schema",
    },
    "shopify": {
        "name": "Shopify",
        "category": "D2C",
        "required_files": ["Orders export Excel/CSV"],
        "guide": "Upload Shopify order export in the common GST Bharat schema",
    },
    "zomato": {
        "name": "Zomato",
        "category": "Food Delivery",
        "required_files": ["Sales report"],
        "guide": "Upload Zomato seller tax report in the common GST Bharat schema",
    },
    "swiggy": {
        "name": "Swiggy",
        "category": "Food Delivery",
        "required_files": ["Sales report"],
        "guide": "Upload Swiggy seller tax report in the common GST Bharat schema",
    },
    "firstcry": {
        "name": "FirstCry",
        "category": "Ecommerce",
        "required_files": ["Sales report"],
        "guide": "Upload FirstCry seller tax report in the common GST Bharat schema",
    },
    "paytm": {
        "name": "Paytm Mall",
        "category": "Ecommerce",
        "required_files": ["Sales report"],
        "guide": "Upload Paytm seller tax report in the common GST Bharat schema",
    },
    "custom": {
        "name": "Custom Excel",
        "category": "Accounting",
...
def save_profile_for_user(
    ...
    *,
    ...
    """Create or update a GST profile without ever replacing a different GSTIN.

    POST semantics:
    - Same user's same GSTIN is treated as an idempotent update.
    - A normal account cannot silently overwrite its existing GSTIN with a new one.
    - Super admins update an existing GSTIN globally instead of creating duplicates.

    PUT semantics:
    - The profile id must be visible/editable to the current user.
    - Duplicate GSTIN rows are rejected for every role.
    """
    gstin, return_period = normalize_profile_payload(payload)
    action = "gst_profile.create"
    ...
    if profile_id is not None:
        ...
        if profile:
            ...
            profile = GSTProfile(user_id=user.id)

    apply_profile_payload(profile, payload, gstin, return_period)
    db.add(profile)
    db.flush()
    audit_gst_profile(user, action, profile, db)
    return profile
...
def create_profile(
    ...
    profile = save_profile_for_user(payload, user, db)
    db.commit()
    db.refresh(profile)
    return profile
...
def list_profiles(
    ...
    return visible_profiles_for_user(user, db)
...
def update_profile(
    ...
    profile = save_profile_for_user(payload, user, db, profile_id=profile_id)
    db.commit()
    db.refresh(profile)
    return profile
